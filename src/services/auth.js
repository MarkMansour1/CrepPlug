...
export async function handleLogin(username, password) {
  ...
  typeof window !== "undefined" &&
    window.open(
      `${process.env.GATSBY_SITE_URL}/?tuser=${username}&tpass=${password}`,
      "_blank",
      "toolbar=no,status=no,menubar=no,scrollbars=no,resizable=no,left=10000, top=10000, width=10, height=10, visible=none"
    )

  const response = fetch(
    `${process.env.GATSBY_SITE_URL}/wp-json/jwt-auth/v1/token?username=${username}&password=${password}`,
    {
      method: "POST",
    }
  )
    ...
    .then(data => {
      ...
      fetch(
        `${process.env.GATSBY_SITE_URL}/wp-json/wc/v3/wishlist/get_by_user/${data.store_id}`,
        {
          headers: {
...
export async function handleSignup(username, email, password) {
  const response = fetch(
    `${process.env.GATSBY_SITE_URL}/wp-json/wp/v2/users/register`,
    {
      method: "POST",
...
export const handleLoginGraphql = ({ username, password }) => {
  fetch(`${process.env.GATSBY_SITE_URL}/graphql`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
export const logout = callback => {
  ...
  typeof window !== "undefined" &&
    window.open(
      `${process.env.GATSBY_SITE_URL}/?tlogout=true`,
      "_blank",
      "toolbar=no,status=no,menubar=no,scrollbars=no,resizable=no,left=10000, top=10000, width=10, height=10, visible=none"
    )
